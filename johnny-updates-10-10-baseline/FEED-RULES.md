...
Log the classification in the Ops Log.
...
Post your full processing plan to the Ops Log (INPUT + CLASSIFY + all planned WRITE entries) BEFORE dispatching the writes to the Writer sub-agent.
...
### Step 6: Verification
The **Verifier** sub-agent runs automatically after your processing completes. It independently:
1. Reads the original feed input
2. Reads your Ops Log entries (what you claimed you wrote)
3. Reads the actual files to confirm each write landed
4. Checks that `clearwater/knowledge/index.md` reflects any new deals, people, knowledge docs, or status changes
5. Checks that `clearwater/knowledge/hot.md` is current if priorities or urgency shifted
6. Posts VERIFIED or FAILED entries to the Ops Log
7. Posts INPUT COMPLETE to the Ops Log
8. Forwards any FAILED entries to Johnny Alerts (chat_id -1003883592748)
